  ...
  >
    <div class="flex flex-col gap-4">
      <button
        ...
        :key="i"
        class="flex items-center justify-between px-6 text-white rounded-full h-11 bg-white-o2"
        @click="
          closeModal('menu');
      ...
      <!-- <NuxtLink
        ...
        class="flex items-center justify-between px-6 text-white uppercase rounded-full h-11 bg-white-o2"
      >
        Вопрос-ответ
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 w-full gap-[9px] sm:gap-4 xl:gap-6 2xl:gap-8">
      ...
      <NuxtLink
        :to="formatPhoneNumber(contacts.phones[1].value)"
        target="_blank"
        class="text-white text-sm 2xl:text-base w-full h-11 sm:h-[54px] 2xl:h-[64px] border-white border rounded-full flex items-center justify-center"
      >
